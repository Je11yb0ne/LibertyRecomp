...
- A Switch VFS preflight audit build can be enabled with `LIBERTY_RECOMP_SWITCH_AUDIT_STOP_AFTER_VFS_PREFLIGHT`; it initializes XAM roots and VFS path mappings, resolves representative common/platform/audio paths, and stops before host startup, module loading, or guest code. This audit intentionally skips the recursive VFS index scan because that scan is a separate Switch filesystem blocker.
