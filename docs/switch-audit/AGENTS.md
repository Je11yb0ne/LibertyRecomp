...
- Continue runtime crash audit from the new baseline:
  - Latest default full ExeFS for Ryujinx smoke tests: `LibertyRecompExefs.nsp`, Build ID `b6e5b56b55b38e03994464386eda9f99ab6cfd19`.
...
- The VFS preflight audit now accepts the user's real extracted Xbox 360 layout where audio lives under `game/xbox360/audio` instead of top-level `game/audio`. It proves `BuildPathCache()`, recursive VFS indexing, and representative path resolution before host startup/module loading/guest code. Next Switch work should attempt a module-load preflight that still stops before guest code until guest memory/page backing is ready.
