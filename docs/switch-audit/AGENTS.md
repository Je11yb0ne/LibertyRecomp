...
- Latest confirmed default full NRO/ExeFS Build ID: `cb986fd2dc5e5554fdf9adfba8353aa2c2836d7d`.
...
- Continue runtime crash audit from the new baseline:
  - Latest default full NRO for optional hardware retest: `LibertyRecomp.nro`, Build ID `cb986fd2dc5e5554fdf9adfba8353aa2c2836d7d`.
...
- For guest memory, the non-NRO ExeFS/NPDM path has now proven `SystemResourceSize` and Alias-region physical mapping in the full process up to a 96 MiB audit window total. Next step is to turn this into a real demand/page-backed guest memory design; a full eager mapping still exceeds the current budget.
