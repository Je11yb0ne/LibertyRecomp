...
- Latest Switch VFS preflight ExeFS Build ID: `9270023bb67df2533a5686d991efa7ad3e9933ed`. It verified the user's real GTA IV layout staged into Ryujinx SD: `BuildPathCache()` returned 200 entries, VFS recursive indexing completed 200 entries, top-level `game/audio` was absent, `audio:` selected `game/xbox360/audio`, and representative `common`, `platform`, and `audio` paths resolved through both `FileSystem::ResolvePath()` and `VFS::Resolve()`. The audit stops before host startup, module loading, and guest code. Switch VFS indexing currently skips nonessential `file_size()` total-byte accounting.
- Latest default full ExeFS rebuild after the VFS index hardening changes: Build ID `b6e5b56b55b38e03994464386eda9f99ab6cfd19`; `LibertyRecompExeFs` build, `readelf -dW`, and a Ryujinx missing-content smoke test were verified. This was not a gameplay test.
- Latest Switch module metadata preflight ExeFS Build ID: `92fe5a27c54121fc0dd73f3bcf81bab270eef09f`. It verified the user's real GTA IV layout staged into Ryujinx SD through Config, install check, XAM roots, `BuildPathCache()`, recursive VFS indexing, `default.xex` read, and lightweight XEX2 metadata parsing. It logged `imageSize=0x11F0000`, `imageBase=0x82000000`, `entry=0x829A0860`, `resource=0x83150000+0x0009BA69`, `fileFormat enc=1 comp=1`, `imports=2`, and `pages=287`, then stopped before full `Image::ParseImage()`, `LdrLoadModule()` guest-memory writes, and `GuestThread::Start()`.
- Latest default full ExeFS rebuild after the module metadata preflight changes: Build ID `06726e9a15c3c4a05df9ed3e35d93961ed5cc581`; `LibertyRecompExeFs` build, `readelf -dW`, and a Ryujinx missing-content smoke test were verified. This was not a gameplay test.
...
- Continue runtime crash audit from the new baseline:
  - Latest default full ExeFS for Ryujinx smoke tests: `LibertyRecompExefs.nsp`, Build ID `06726e9a15c3c4a05df9ed3e35d93961ed5cc581`.
...
- The VFS preflight audit now accepts the user's real extracted Xbox 360 layout where audio lives under `game/xbox360/audio` instead of top-level `game/audio`. It proves `BuildPathCache()`, recursive VFS indexing, and representative path resolution before host startup/module loading/guest code.
- The module metadata preflight now proves `default.xex` can be read and its lightweight XEX2 metadata parsed before guest-memory writes. A full `Image::ParseImage()` attempt reached the parse call after reading `11841536` bytes but did not return within a 240-second Ryujinx window, so the next Switch boundary should instrument or replace the full image parse path without entering guest code.
