...
- Full `Image::ParseImage()` is not yet a stable Switch audit boundary. Ryujinx reached `Switch module-load preflight audit: parsing module image.` after reading `11841536` bytes from `default.xex`, then did not reach a summary within the 240-second test window. Switch-local phase instrumentation later proved in-place AES-CBC decrypt of the payload completes, but basic decompression needs a `0x11F0000` output buffer and currently fails under the 16 MiB libnx heap while the loaded XEX buffer is still resident. Continue with a Switch-safe module-loader memory strategy before calling or replacing the full image parse path.
