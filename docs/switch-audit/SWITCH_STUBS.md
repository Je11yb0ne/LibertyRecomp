...
- `LIBERTY_RECOMP_SWITCH_AUDIT_STOP_AFTER_MODULE_LOAD_PREFLIGHT` is a Switch-only audit stop that runs `Config::Load()`, checks `default.xex`, builds the host path cache, registers XAM roots, initializes VFS with the recursive index scan, reads `default.xex`, validates lightweight XEX2 header/security/file-format/resource/import metadata, and stops before full `Image::ParseImage()`, `LdrLoadModule()` guest-memory writes, video/audio setup, `GuestThread::Start()`, or generated PPC code. It exists to isolate module metadata only and is not a release/runtime feature.
