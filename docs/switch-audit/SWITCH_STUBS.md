...
- `LIBERTY_RECOMP_SWITCH_AUDIT_STOP_AFTER_VFS_PREFLIGHT` is a Switch-only audit stop that runs `Config::Load()`, checks `default.xex`, registers XAM roots, initializes VFS mappings without the recursive index scan, resolves representative common/platform/audio paths, and stops before host startup, module loading, or guest code. It exists to isolate root/path plumbing only and is not a release/runtime feature.
...
- `VFS::Initialize(extractedRoot, false)` is used only by the VFS preflight audit to skip recursive indexing. The normal `VFS::Initialize(extractedRoot)` path still builds the index. The recursive index scan on Switch/Ryujinx remains a separate filesystem blocker to harden before normal startup uses it.
