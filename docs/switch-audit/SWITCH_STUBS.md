...
- Continue ordinary Switch runtime audits with `LibertyRecompExeFs` / `LibertyRecompExefs.nsp`. Build `LibertyRecompNro` only when testing Homebrew Menu / NRO icon, name, or NRO-specific launch behavior.
...
- `LIBERTY_RECOMP_SWITCH_AUDIT_STOP_AFTER_INSTALL_CHECK` is a Switch-only audit stop that runs `Config::Load()`, calls `Installer::checkGameInstall(GetGamePath(), modulePath)`, logs whether `sdmc:/switch/LibertyRecomp/game/default.xex` exists, and then stops before host startup, update checking, module loading, or guest code. It exists to isolate SD content-path plumbing and is not a release/runtime feature.
- `LIBERTY_RECOMP_SWITCH_AUDIT_STOP_AFTER_CONTENT_LAYOUT_CHECK` is a Switch-only audit stop that runs `Config::Load()`, calls `Installer::checkGameInstall(GetGamePath(), modulePath)`, checks the expected extracted SD layout (`game/default.xex`, `game/common`, `game/xbox360`, top-level `game/audio` or platform `game/xbox360/audio`), optional source RPF archives, legacy `RPF DUMP`, and optional `dlc`, then stops before host startup, VFS initialization, module loading, or guest code. It exists to isolate SD content layout only and is not a release/runtime feature.
- `LIBERTY_RECOMP_SWITCH_AUDIT_STOP_AFTER_VFS_PREFLIGHT` is a Switch-only audit stop that runs `Config::Load()`, checks `default.xex`, builds the host path cache, registers XAM roots, initializes VFS with the recursive index scan, resolves representative common/platform/audio paths, and stops before host startup, module loading, or guest code. It exists to isolate root/path/index plumbing only and is not a release/runtime feature.
- `LIBERTY_RECOMP_SWITCH_AUDIT_STOP_AFTER_MODULE_LOAD_PREFLIGHT` is a Switch-only audit stop that runs `Config::Load()`, checks `default.xex`, builds the host path cache, registers XAM roots, initializes VFS with the recursive index scan, reads `default.xex`, validates lightweight XEX2 header/security/file-format/resource/import metadata, then runs a Switch-local full-parse phase probe. The probe decrypts the XEX payload in place and uses a staged basic-decompression view to scan PE sections and import thunks without allocating the full `0x11F0000` decompressed image. When sparse guest memory is enabled, it logs and touch-probes the bounded `LdrLoadModule()` guest ranges, materializes the staged XEX image directly into the mapped guest image span `0x82000000..0x831F0000` block-by-block, validates/initializes the XDBF resource wrapper, zeroes the collision range, initializes the stream struct, zeroes worker globals, and verifies the copied data/zero-fill/side-effect writes. It still stops before video/audio setup, `GuestThread::Start()`, or generated PPC code. It exists to isolate host-side module loader boundaries only and is not a release/runtime feature.
- `main.cpp` currently selects `game/xbox360/audio` as the Switch `audio:` root when top-level `game/audio` is absent. `kernel/vfs.cpp` has matching fallback mappings for `audio.rpf`, `audio/`, and `sfx/`. This is layout plumbing only; it does not implement XMA decoding or runtime RPF archive reading.
- `VFS::Initialize(extractedRoot)` now completes the recursive index scan in the Switch VFS preflight audit with the user's real extracted layout. The Switch scan uses non-throwing recursive iteration and skips `std::filesystem::file_size()` size accounting because Ryujinx/libnx aborted on that metadata path while scanning SD content. This is acceptable for path lookup auditing because the index only needs normalized path keys; final runtime size/stat behavior still needs a broader Switch filesystem policy.
- Full `Image::ParseImage()` is not yet a stable Switch audit boundary. Ryujinx reached `Switch module-load preflight audit: parsing module image.` after reading `11841536` bytes from `default.xex`, then did not reach a summary within the 240-second test window. Switch-local phase instrumentation later proved in-place AES-CBC decrypt of the payload completes; a duplicate decrypted-buffer allocation can enter the GCC unwinder path, and a full `0x11F0000` decompression output allocation fails under the 16 MiB libnx heap. The current staged basic-decompression view avoids that allocation, scans `13` PE sections plus `2` import libraries / `484` thunk descriptors with `0` missing thunk targets, materializes the staged image into mapped guest memory without a host-side full-image allocation, and verifies the XDBF/resource/collision/stream/worker pre-guest loader side effects. Do not enter `GuestThread::Start()` or generated PPC code until the guide explicitly allows it.
...
- A module-image guest-memory preflight showed that retaining the full scanned image/function-table mapping can fail in the module preflight profile at `image/function table guest=0x82000000 size=0x24A0000` with `0x0000D001`, leaving `Memory::base=null`. The narrower module-image profile sets `LIBERTY_RECOMP_SWITCH_GUEST_IMAGE_TABLE_AUDIT_SIZE=0x11F0000` and `LIBERTY_RECOMP_SWITCH_GUEST_MEMORY_AUDIT_SKIP_FUNCTION_MAPPINGS=ON`; Ryujinx confirmed it maps `0x82000000..0x831F0000` and successfully touch-probes the image, resource, collision, stream, and worker-global ranges before `GuestThread::Start()`.
